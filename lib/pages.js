...
function editionCard(book, e) {
  const signed = e.kind === 'signed';
  const low = signed && e.stock !== null && e.stock > 0 && e.stock <= 5;

  return `
      <li class="buy">
        <div class="buy__head">
          <h3>${signed ? 'Signed copy' : 'Standard copy'}</h3>
          <p class="buy__price">${esc(money(e.price_cents, e.currency))}</p>
        </div>
        <p class="buy__note">${signed
        ...
        <form class="buy__form" method="POST" action="/api/checkout">
          ...
          <input class="buy__qty" id="qty-${esc(e.id)}" name="quantity" type="number"
                 value="1" min="1" max="${signed && e.stock ? e.stock : 10}" inputmode="numeric">
          <button class="btn ${signed ? 'btn--gold' : 'btn--navy'}" type="submit">
